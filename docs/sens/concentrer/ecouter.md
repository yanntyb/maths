...
> [Retour au sens Concentrer](README.md) | [Exemples](exemples.md) | [Godel](../../meta-objets/)
...
| 4 | [Produit lineaire](../ponderer/) | `R x R -> R` | ponderation spatiale : `w(x) * f(x)` en chaque point |
| 5 | [Integration](../observer/projeter.md) sur `Omega` | `Omega x (Omega -> R) -> R` | somme ponderee sur la salle = volume capte |
| 6 | [Produit lineaire](../ponderer/) currie | `R -> (R -> R)` | `<vol, _>` : modulateur parametre par le volume capte |
...
La fenetre `w` est le levier de [concentration](README.md). Quand `epsilon` retrecit, le micro zoome du global au local — voir la [fiche concentrer](README.md) pour la construction et la limite delta de Dirac.

---

## Phase 2 -- Modulation : `<volume capte, .>`

Le circuit ne s'arrete pas au volume capte. Le scalaire `vol : R` devient la premiere entree d'un nouveau [produit lineaire](../ponderer/). Via [currying](../../meta-objets/currying.md), fixer `vol` produit un **modulateur** :
...
| `1/d^2` | attenuation par la distance (comme dans [projeter](../observer/projeter.md)) |
...
Par [currying du produit lineaire](../ponderer/#currying-du-produit-lineaire), on fixe `p_i^{a_i}` comme premiere entree. Chaque `curry(*)(p_i^{a_i})` est un **amplificateur** `R -> R` qui multiplie un signal par `p_i^{a_i}`.
...
|-------|----------------|----------------------|
| [Produit lineaire](../ponderer/) `R x R -> R` | 3 | ponderation spatiale, modulation, amplificateur currie |
| [Produit de dualite](../observer/) `E* x E -> R` | 1 | limite de la concentration (`delta_{x_0}`) |
