...
export function drawAxiomSphere(ctx, rect, proof, classified, tri) {
  ...
  ordered.forEach(it => {
    ...
    if (it.active) {
      ...
      // Axiome structurellement inactif — marquer avec tirets visibles
      p.noFill();
      p.stroke('#D1D5DB');
      p.strokeWeight(4);  // Plus épais pour être visible
      p.drawingContext.setLineDash([3, 5]);  // Tirets plus espacés
      p.arc(cx, cy, R * 2, R * 2, a0, a1);
      ...
      p.text(it.entry.code + ' —', lx, ly);  // Ajouter trait pour "exclu"
    }
